module.exports = {
  ...
  GET_DOCTOR_FAQS:
    "SELECT faq_id AS faqId, doctor_id AS doctorId, answer, question, COUNT(*) OVER() AS totalRows FROM doctor_faqs WHERE doctor_id = ? ORDER BY created_at DESC LIMIT ?,?",
  GET_DOCTOR_FAQ:
  ...
  GET_ACTIVE_DOCTOR_FAQS:
    "SELECT faq_id AS faqId, doctor_id AS doctorId, answer, question, COUNT(*) OVER() AS totalRows FROM doctor_faqs WHERE doctor_id = ? AND is_active = 1 ORDER BY created_at DESC LIMIT ?,?",
  COUNT_DOCTOR_FAQ:
