...
exports.view_blog = (req, res, next) => {
	...
	connectPool.query(					  
            'select * from blog order by id DESC', (error, results, fields) => {
			if (error) {
...
exports.add_blog = (req, res, next) => {
	connectPool.query(
            'select * from blog_category where status=1 order by id DESC', (error, results, fields) => {
			if (error) {
		...
		if (req.file) {
			...
			validator(req.body, validationRule, {}, (err, status) => {
				if(status)
				{
					...
					var insQry = "INSERT INTO blog SET category_id = '" + category_id + "',name = '" + name + "',description = '" + description + "', image = '" + prof_pic + "', tags = '" + tags + "'";
...
exports.edit_blog = (req, res) => {
	...
	let sql = 'SELECT * FROM blog WHERE id = ' + id + ' LIMIT 1';
        connectPool.query(sql, (error, results, fields) => {
			...
			connectPool.query(
				'select * from blog_category where status=1 order by id DESC', (errors, result, fields) => {
				if (errors) {
...
exports.delete_blog = (req, res) => {
	...
	let sql = 'SELECT * FROM blog WHERE id = ' + id + ' LIMIT 1';
        connectPool.query(sql, (error, results, fields) => {
			...
			if (results.length > 0) {
				connectPool.query(
					'DELETE FROM blog WHERE id = "' + id + '" ', (errors, result, fields) => {
						if (errors) {
							return res.render('view_blog', {
								title: 'Manage Blog',
								error: errors
