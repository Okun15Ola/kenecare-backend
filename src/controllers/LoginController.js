		...
		if(status)
		{
					...
					try {       
						...
						if (email) {
							...
							connectPool.query('SELECT * FROM admin WHERE email = ?', [email], function(error, results, fields) {
								if (error) {
		...
		if(status)
		{
			...
			connectPool.query('SELECT * FROM admin WHERE email = ?', [email], function(error, results, fields) {
				...
				if (results.length > 0) {
					...
					connectPool.query(sqlupdate, fieldsToUpdate, function(errors, result){
						if (errors) {
						   req.flash('error', errors);
							return res.redirect('/forgot_password/');
						}
						var linkred=BASE_URL+'reset_password/'+token;
						var html='<p>Please click on below password reset link <a target="_blank" href="'+linkred+'">Reset Password</a></p>';
