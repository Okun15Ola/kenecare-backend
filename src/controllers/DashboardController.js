			...
			connectPool.query(sql2, (error2, results2, fields2) => {
				...
				connectPool.query(sql3, (error3, results3, fields3) => {
					...
					connectPool.query(sql4, (error4, results4, fields4) => {
							...
							for (var x = 0; x < results4.length; x++) {
								...
								if (results4[x].image != '') {
									...
									if (fs.existsSync(imagePath)) {
										var img = BASE_URL + 'public/upload/user/' + results4[x].image;
									}
								}
						...
						var sql5 = 'select a.*, b.name as specialty from user a LEFT JOIN specialization b ON a.specialization_id = b.id where a.type=2 order by a.id DESC LIMIT 5';
							...
							if(results5.length > 0){
								...
								for (var x = 0; x < results5.length; x++) {
									...
									if (results5[x].image != '') {
										...
										if (fs.existsSync(imagePath)) {
											var img = BASE_URL + 'public/upload/user/' + results5[x].image;
										}
									}
...
exports.profile = (req, res) => {
	connectPool.query(
            ...
            session_user_id, (error, results, fields) => {
            if (error) {
				req.flash('error', error);
				return res.redirect('/dashboard');
            }
