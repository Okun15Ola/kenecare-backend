...
exports.view_testimonial = (req, res, next) => {
	connectPool.query(
            'select * from testimonial order by id DESC', (error, results, fields) => {
			if (error) {
		...
		if (req.file) {
			...
			validator(req.body, validationRule, {}, (err, status) => {
				if(status)
				{
					...
					var insQry = "INSERT INTO testimonial SET name = '" + name + "',description = '" + description + "', image = '" + prof_pic + "'";
...
exports.delete_testimonial = (req, res) => {
	...
	let sql = 'SELECT * FROM testimonial WHERE id = ' + id + ' LIMIT 1';
        connectPool.query(sql, (error, results, fields) => {
			...
			if (results.length > 0) {
				connectPool.query(
					'DELETE FROM testimonial WHERE id = "' + id + '" ', (errors, result, fields) => {
						if (errors) {
							return res.render('view_testimonial', {
								title: 'Manage Testimonial',
								error: errors
						...
						var imagePath = 'public/upload/testimonial/'+results[0].image;
...
exports.do_edit_testimonial = (req, res) => {
	...
	if (req.file) 
	{
		...
		if(status)
		{
			let sql = 'SELECT * FROM testimonial WHERE id = ' + req.body.id + ' LIMIT 1';
