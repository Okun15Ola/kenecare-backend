...
exports.view_department_type = (req, res, next) => {
	connectPool.query(
            'select * from department_type order by id DESC', (error, results, fields) => {
			if (error) {
		...
		if (req.file) {
			...
			validator(req.body, validationRule, {}, (err, status) => {
				if(status)
				{
					...
					var insQry = "INSERT INTO department_type SET name = '" + name + "',description = '" + description + "', image = '" + prof_pic + "'";
					...
					id = results.insertId;
					req.flash('success', 'Department Type has been saved successfully!')
					return res.redirect('/view_department_type');
				});
				}else{
...
exports.delete_department_type = (req, res) => {
	...
	let sql = 'SELECT * FROM department_type WHERE id = ' + id + ' LIMIT 1';
        connectPool.query(sql, (error, results, fields) => {
			...
			if (results.length > 0) {
				connectPool.query(
					'DELETE FROM department_type WHERE id = "' + id + '" ', (errors, result, fields) => {
						if (errors) {
							return res.render('view_department_type', {
								title: 'Manage Department Type',
								error: errors
						...
						var imagePath = 'public/upload/department_type/'+results[0].image;
...
exports.do_edit_department_type = (req, res) => {
	...
	if (req.file) 
	{
		...
		if(status)
		{
			let sql = 'SELECT * FROM department_type WHERE id = ' + req.body.id + ' LIMIT 1';
        connectPool.query(sql, (errors, result, fields) => {
			...
			if (result.length > 0) {
				var imagePath = 'public/upload/department_type/'+result[0].image;
				if (fs.existsSync(imagePath)) {
					fs.unlinkSync(imagePath);
				}
				var fieldsToUpdate={};
				...
				fieldsToUpdate['image']=req.file.filename;
					var sqlupdate = 'UPDATE department_type SET ? WHERE id  = ' + req.body.id;
					connectPool.query(sqlupdate, fieldsToUpdate, function(error, results){
						if (error) {
						   req.flash('error', error);
							return res.redirect('/edit_department_type/' + req.body.id);
						}
						req.flash('success', 'Department Type has been updated successfully!')
						return res.redirect('/view_department_type');
					});
			}else{
				return res.redirect('/view_department_type');	
