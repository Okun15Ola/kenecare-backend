...
exports.view_blog_category = (req, res, next) => {
	connectPool.query(
            'select * from blog_category order by id DESC', (error, results, fields) => {
			if (error) {
...
exports.delete_blog_category = (req, res) => {
	...
	let sql = 'SELECT * FROM blog_category WHERE id = ' + id + ' LIMIT 1';
        connectPool.query(sql, (error, results, fields) => {
			...
			if (results.length > 0) {
				connectPool.query(
					'DELETE FROM blog_category WHERE id = "' + id + '" ', (errors, result, fields) => {
						if (errors) {
							return res.render('view_blog_category', {
								title: 'Manage Category',
								error: errors
...
exports.do_edit_blog_category = (req, res) => {
		...
		if(status)
		{
			let sql = 'SELECT * FROM blog_category WHERE id = ' + req.body.id + ' LIMIT 1';
        connectPool.query(sql, (errors, result, fields) => {
			...
			if (result.length > 0) {
				...
				fieldsToUpdate['name']=req.body.name;
					...
					var sqlupdate = 'UPDATE blog_category SET ? WHERE id  = ' + req.body.id;
